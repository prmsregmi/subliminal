...
export function ProductRail({
  ...
  return (
    <div className="flex flex-col gap-1">
      <div className="label-eyebrow px-2 pb-1">Projects</div>
      {isLoading && (
      ...
      {products?.length === 0 && (
        <div className="px-2 py-3 text-xs text-muted-foreground">
          Submit a company website to start a project for this profile.
        </div>
      )}
      {products?.map((p) => (
        <button
          ...
          className={cn(
            "group flex flex-col gap-0.5 rounded-lg border px-2.5 py-2 text-left transition-all",
            activeId === p._id
              ? "border-primary/30 bg-primary/10 shadow-sm"
              : "border-transparent hover:border-primary/20 hover:bg-secondary/70 hover:shadow-sm",
          )}
        >
